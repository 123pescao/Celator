/**
 * Celator error codes — used in Result<T, ErrorCode> types and audit logs.
 */

export const ErrorCodes = {
  // Auth / access
  UNAUTHORIZED: 'UNAUTHORIZED',
  FORBIDDEN: 'FORBIDDEN',
  MFA_REQUIRED: 'MFA_REQUIRED',
  MFA_STALE: 'MFA_STALE',
  SESSION_LIMIT_EXCEEDED: 'SESSION_LIMIT_EXCEEDED',

  // Vault
  VAULT_RECORD_NOT_FOUND: 'VAULT_RECORD_NOT_FOUND',
  VAULT_CRYPTO_SHREDDED: 'VAULT_CRYPTO_SHREDDED',
  VAULT_PURPOSE_DENIED: 'VAULT_PURPOSE_DENIED',
  VAULT_ENCRYPT_FAILED: 'VAULT_ENCRYPT_FAILED',
  VAULT_DECRYPT_FAILED: 'VAULT_DECRYPT_FAILED',
  VAULT_FIELD_TYPE_INVALID: 'VAULT_FIELD_TYPE_INVALID',
  DEK_NOT_FOUND: 'DEK_NOT_FOUND',

  // PII policy
  PII_FORBIDDEN_IN_REDACTED_PREVIEW: 'PII_FORBIDDEN_IN_REDACTED_PREVIEW',

  // Approval gate
  APPROVAL_GATE_BLOCKED: 'APPROVAL_GATE_BLOCKED',
  SNAPSHOT_NOT_FOUND: 'SNAPSHOT_NOT_FOUND',
  SNAPSHOT_EXPIRED: 'SNAPSHOT_EXPIRED',
  SNAPSHOT_INVALID: 'SNAPSHOT_INVALID',
  SNAPSHOT_SIGNATURE_INVALID: 'SNAPSHOT_SIGNATURE_INVALID',
  PAYLOAD_HASH_MISMATCH: 'PAYLOAD_HASH_MISMATCH',

  // Consent
  CONSENT_ACTION_DENIED: 'CONSENT_ACTION_DENIED',
  CONSENT_MAPPING_IMMUTABLE: 'CONSENT_MAPPING_IMMUTABLE',
  AUTHORIZATION_NOT_FOUND: 'AUTHORIZATION_NOT_FOUND',
  AUTHORIZATION_REVOKED: 'AUTHORIZATION_REVOKED',
  AUTHORIZATION_EXPIRED: 'AUTHORIZATION_EXPIRED',

  // State machine
  TRANSITION_NOT_ALLOWED: 'TRANSITION_NOT_ALLOWED',

  // Emergency pause
  EMERGENCY_PAUSE_ACTIVE: 'EMERGENCY_PAUSE_ACTIVE',
  PAUSE_NOT_FOUND: 'PAUSE_NOT_FOUND',
  PAUSE_NOT_ACTIVE: 'PAUSE_NOT_ACTIVE',

  // Client
  CLIENT_NOT_FOUND: 'CLIENT_NOT_FOUND',
  CLIENT_NOT_ACTIVE: 'CLIENT_NOT_ACTIVE',
  CLIENT_IDENTITY_UNVERIFIED: 'CLIENT_IDENTITY_UNVERIFIED',

  // Data source target registry
  DATA_SOURCE_TARGET_NOT_FOUND: 'DATA_SOURCE_TARGET_NOT_FOUND',

  // Manual removal submissions
  MANUAL_SUBMISSION_NOT_FOUND: 'MANUAL_SUBMISSION_NOT_FOUND',
  MANUAL_SUBMISSION_INVALID_STATUS: 'MANUAL_SUBMISSION_INVALID_STATUS',

  // Workflow engine
  PLAYBOOK_NOT_FOUND: 'PLAYBOOK_NOT_FOUND',
  WORKFLOW_RUN_NOT_FOUND: 'WORKFLOW_RUN_NOT_FOUND',
  WORKFLOW_STEP_NOT_FOUND: 'WORKFLOW_STEP_NOT_FOUND',
  WORKFLOW_INVALID_TRANSITION: 'WORKFLOW_INVALID_TRANSITION',
  WORKFLOW_UNSAFE_TEXT: 'WORKFLOW_UNSAFE_TEXT',

  // Removal request packets (Phase 2B)
  PACKET_NOT_FOUND: 'PACKET_NOT_FOUND',
  PACKET_INVALID_STATUS: 'PACKET_INVALID_STATUS',

  // Follow-up reminders (Phase 2C)
  FOLLOW_UP_NOT_FOUND: 'FOLLOW_UP_NOT_FOUND',
  FOLLOW_UP_INVALID_STATUS: 'FOLLOW_UP_INVALID_STATUS',

  // Evidence records (Phase 2C)
  EVIDENCE_RECORD_NOT_FOUND: 'EVIDENCE_RECORD_NOT_FOUND',
  EVIDENCE_RECORD_IMMUTABLE: 'EVIDENCE_RECORD_IMMUTABLE',
  EVIDENCE_TYPE_INVALID: 'EVIDENCE_TYPE_INVALID',

  // Client intake (Phase 3B)
  INTAKE_SESSION_NOT_FOUND: 'INTAKE_SESSION_NOT_FOUND',
  INTAKE_INVALID_STATUS: 'INTAKE_INVALID_STATUS',

  // Notifications (Phase 3C)
  NOTIFICATION_NOT_FOUND: 'NOTIFICATION_NOT_FOUND',
  NOTIFICATION_INVALID_STATUS: 'NOTIFICATION_INVALID_STATUS',
  NOTIFICATION_SUBJECT_UNSAFE: 'NOTIFICATION_SUBJECT_UNSAFE',
  NOTIFICATION_BODY_UNSAFE: 'NOTIFICATION_BODY_UNSAFE',

  // Reports (Phase 4A)
  REPORT_NOT_FOUND: 'REPORT_NOT_FOUND',
  REPORT_INVALID_STATUS: 'REPORT_INVALID_STATUS',
  REPORT_CONTENT_UNSAFE: 'REPORT_CONTENT_UNSAFE',
  REPORT_EXPIRED: 'REPORT_EXPIRED',

  // Automation plans (Phase 4B)
  AUTOMATION_PLAN_NOT_FOUND: 'AUTOMATION_PLAN_NOT_FOUND',
  AUTOMATION_PLAN_INVALID_STATUS: 'AUTOMATION_PLAN_INVALID_STATUS',
  AUTOMATION_PLAN_PAUSED: 'AUTOMATION_PLAN_PAUSED',
  AUTOMATION_PLAN_INTENT_UNSAFE: 'AUTOMATION_PLAN_INTENT_UNSAFE',

  // General
  NOT_FOUND: 'NOT_FOUND',
  VALIDATION_ERROR: 'VALIDATION_ERROR',
  INTERNAL_ERROR: 'INTERNAL_ERROR',
  AUDIT_LOG_FAILED: 'AUDIT_LOG_FAILED',
} as const;

export type ErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

/**
 * CelatorError — base error class for programming errors (not policy decisions).
 * Policy decisions should use Result<T, ErrorCode> instead.
 */
export class CelatorError extends Error {
  constructor(
    public readonly code: ErrorCode,
    message: string,
    public readonly context?: Record<string, unknown>,
  ) {
    super(message);
    this.name = 'CelatorError';
  }
}
