/**
 * playbook-definitions.ts — Static definitions for generic removal playbooks.
 *
 * No real client PII. No credentials. No scraping instructions.
 * All instructions are generic and PII-safe operator guidance.
 * Imported by both seed-dev-playbooks.ts and unit tests.
 */
    ...
    steps: [
      {
        stepOrder: 1,
        stepKind: 'VERIFY_TARGET_REQUIREMENTS',
        title: 'Verify People-Search Listing Presence',
        instructions:
          'Confirm the people-search target is active and an opt-out process is published. Verify the site has a public opt-out or removal form. Do not access client vault data at this step.',
      },
      {
        stepOrder: 2,
        stepKind: 'PREPARE_REMOVAL_DRAFT',
        title: 'Prepare Removal Draft',
        instructions:
          'Use the removal draft service to review the required PII fields (typically full name and location). Prepare a redacted draft summary. Confirm client authorization covers the removal action type.',
      },
      {
        stepOrder: 3,
        stepKind: 'OPERATOR_REVIEW',
        title: 'Operator Review',
        instructions:
          'Review the opt-out scope and confirm the client authorization is active and covers the data type. Check for high-risk flags. Approve or escalate.',
        requiresApproval: true,
      },
      {
        stepOrder: 4,
        ...
        title: 'Submit Opt-Out Request',
        instructions:
          'Navigate to the site opt-out form. Use the vault access flow for form field completion only — do not store raw PII in notes. Submit the form and record any confirmation ID or email acknowledgment.',
        requiresManualSubmission: true,
      },
      {
        stepOrder: 5,
        ...
        instructions:
          'Allow the site standard processing time (typically 5-30 days). People-search opt-outs may require email verification from the operator inbox. Monitor for follow-up email from the site.',
      },
      {
        stepOrder: 6,
        stepKind: 'RECORD_OUTCOME',
        ...
        instructions:
          'Update the submission with the site response. Record any confirmation reference in the safe result summary. If the opt-out requires identity verification, document next steps without storing raw PII.',
      },
      {
        stepOrder: 7,
        stepKind: 'VERIFY_REMOVAL',
        title: 'Verify Listing Removed',
        instructions:
          'Search the people-search site to confirm the listing is no longer publicly visible. Use only publicly accessible search. Record verification outcome.',
      },
      {
        stepOrder: 8,
        stepKind: 'CLOSE_TASK',
        title: 'Close Task',
        instructions:
          'Update the cleanup task with the verified removal outcome. If listing persists after the processing window, escalate or schedule a follow-up check.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        stepOrder: 1,
        stepKind: 'VERIFY_TARGET_REQUIREMENTS',
        title: 'Verify Exposure and Target',
        instructions:
          'Confirm the target site is active and that a removal or correction request pathway is available (contact form, DMCA takedown, or privacy request form). Do not access vault data at this step.',
      },
      {
        stepOrder: 2,
        stepKind: 'PREPARE_REMOVAL_DRAFT',
        title: 'Prepare Removal Request Draft',
        instructions:
          'Use the removal draft service to confirm which contact fields are exposed and available in the vault. Prepare a redacted summary of the removal request. Confirm client authorization scope.',
      },
      {
        stepOrder: 3,
        stepKind: 'OPERATOR_REVIEW',
        title: 'Operator Review',
        instructions:
          'Review the removal request and confirm the exposed data type aligns with the client authorization. Check for high-risk flags. Approve or escalate.',
        requiresApproval: true,
      },
      {
        stepOrder: 4,
        ...
        title: 'Submit Removal Request',
        instructions:
          'Submit the removal request via the site contact form, privacy portal, or DSAR email. Use the vault access flow for any required field values. Record the submission reference or confirmation.',
        requiresManualSubmission: true,
      },
      {
        stepOrder: 5,
        stepKind: 'WAIT_FOR_CONFIRMATION',
        title: 'Wait for Site Response',
        instructions:
          'Allow standard response time per the site privacy policy (typically 30 days). Monitor for operator inbox replies. Record any request reference provided.',
      },
      {
        stepOrder: 6,
        stepKind: 'RECORD_OUTCOME',
        title: 'Record Outcome',
        instructions:
          'Update the submission with the site response. Record any reference code in the safe result summary. If no response, document for potential escalation.',
      },
      {
        stepOrder: 7,
        stepKind: 'VERIFY_REMOVAL',
        title: 'Verify Removal',
        instructions:
          'Check the site to confirm the contact information is no longer publicly visible. Use only publicly accessible pages. Record verification result.',
      },
      {
        stepOrder: 8,
        stepKind: 'CLOSE_TASK',
        title: 'Close Task',
        instructions:
          'Update the cleanup task with the verified outcome. If data persists, escalate or schedule a follow-up. Add a safe result summary.',
      },
    ],
  },
];
